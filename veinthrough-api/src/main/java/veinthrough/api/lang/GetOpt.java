...
public class GetOpt {
     ...
     */
    private List<String> fileNameArguments;
    /**
    ...
    public GetOpt(final String opt) {
        ...
        for (int i = 0, ix = 0; i < opt.length(); i++) {
            ...
            log.debug(methodLog("CONSTR: options[" + ix + "] = " + c + ", " + argTakesValue));
     ...
     * @return The parsed arguments, a Map whose keys are Strings of length 1 (containing the char
    ...
    public Map<String, String> parseArguments(String[] argv) {
        ...
        for (int i = 0; i < argv.length; i++) { // Cannot use foreach, need i
            log.debug(methodLog("parseArg: i=" + i + ": arg " + argv[i]));
     ...
     */
    private char getopt(String[] argv) {
        log.debug(methodLog(
